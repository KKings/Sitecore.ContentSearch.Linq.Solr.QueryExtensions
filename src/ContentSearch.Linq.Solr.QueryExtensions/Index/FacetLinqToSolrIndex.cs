...
namespace Sitecore.ContentSearch.Linq.Solr.Index
{
    ...
    using ISettings = Abstractions.ISettings;
    ...
    public class FacetLinqToSolrIndex<TItem> : LinqToSolrIndex<TItem>
    {
        ...
        internal struct SolrSearchResults<TElement>
        {
            ...
            {
                if (index < 0 || index > this._searchResults.Count)
                    throw new IndexOutOfRangeException();
                return this._mapper.MapToType<TElement>(this._searchResults[index], this._selectMethod, this._virtualFieldProcessors, this._executionContexts, this._context.SecurityOptions);
            }
            ...
            {
                if (this._searchResults.Count < 1)
            ...
            public TElement Last()
            {
                if (this._searchResults.Count < 1)
                    throw new InvalidOperationException("Sequence contains no elements");
                return this.ElementAt(this._searchResults.Count - 1);
            }
            ...
            public TElement Single()
            {
                ...
                return this._mapper.MapToType<TElement>(this._searchResults[0], this._selectMethod, this._virtualFieldProcessors, this._executionContexts, this._context.SecurityOptions);
            }
            ...
            {
                foreach (var document in this._searchResults)
                {
                    var score = -1f;
                    object scoreObj;
                    if (document.TryGetValue("score", out scoreObj) && scoreObj is float)
                        score = (float)scoreObj;
                    yield return new SearchHit<TElement>(score, this._mapper.MapToType<TElement>(document, this._selectMethod, this._virtualFieldProcessors, this._executionContexts, this._context.SecurityOptions));
                }
            }
            ...
            {
                foreach (var document in this._searchResults)
                    yield return this._mapper.MapToType<TElement>(document, this._selectMethod, this._virtualFieldProcessors, this._executionContexts, this._context.SecurityOptions);
            }

            public Dictionary<string, ICollection<KeyValuePair<string, int>>> GetFacets()
            {
                var facetFields = this._searchResults.FacetFields;
                var facetPivots = this._searchResults.FacetPivots;
            ...
            private ICollection<KeyValuePair<string, int>> Flatten(IEnumerable<Pivot> pivots, string parentName)
            {
                ...
                foreach (var pivot in pivots)
                {
                    ...
                    if (pivot.HasChildPivots)
                        hashSet.UnionWith(this.Flatten(pivot.ChildPivots, pivot.Value));
                }
                return hashSet;
            }
        }
